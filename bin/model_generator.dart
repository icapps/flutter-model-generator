...
Future<void> main(List<String> args) async {
  ...
  final swagerGeneratorConfigFile =
      File(join(Directory.current.path, 'swagger_generator', 'config.yaml'));
  if (!swagerGeneratorConfigFile.existsSync()) {
    throw Exception(
        'This program requires a config file. `swagger_generator/config.yaml`');
  }
  final swaggerGeneratorContent = swagerGeneratorConfigFile.readAsStringSync();
  final swaggerGeneratorConfig =
      SwaggerGeneratorConfig(swaggerGeneratorContent);
...
void writeToFiles(PubspecConfig pubspecConfig,
